...
export class Executor {
  ...
  private formFieldResolver: ((value: string) => void) | null = null;
  ...
  resolveFormField(value: string): void {
    if (this.formFieldResolver) {
      this.formFieldResolver(value);
      this.formFieldResolver = null;
    }
  }

  waitForFormField(timeoutMs = 120000): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      this.formFieldResolver = resolve;
      setTimeout(() => {
        if (this.formFieldResolver === resolve) {
          this.formFieldResolver = null;
          reject(new Error('Form field response timed out'));
        }
      }, timeoutMs);
    });
  }
