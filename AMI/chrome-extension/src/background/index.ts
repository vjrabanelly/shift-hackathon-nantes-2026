...
import { DialogueLlmService } from './services/dialogueLlm';
      ...
      try {
        switch (message.type) {
          ...
          case 'dialogue_chat': {
            try {
              if (!message.messages) {
                return port.postMessage({ type: 'dialogue_chat_error', error: 'No messages provided' });
              }
              ...
              if (!dialogueModel) {
                return port.postMessage({
                  type: 'dialogue_chat_error',
                  error: t('bg_cmd_dialogue_noModel'),
                });
              }

              const dialogueService = DialogueLlmService.create(providers, agentModels);
              const response = await dialogueService.chat(message.messages);

              return port.postMessage({
                type: 'dialogue_chat_result',
                ...response,
              });
            } catch (error) {
              ...
              return port.postMessage({
                type: 'dialogue_chat_error',
                error: error instanceof Error ? error.message : t('bg_cmd_dialogue_failed'),
              });
            }
          }

          case 'dialogue_narrate': {
            try {
              const providers = await llmProviderStore.getAllProviders();
              const agentModels = await agentModelStore.getAllAgentModels();
              const dialogueService = DialogueLlmService.create(providers, agentModels);
              const result = await dialogueService.narrate({
                events: message.events || [],
                taskDescription: message.taskDescription || '',
                previousNarration: message.previousNarration,
              });

              return port.postMessage({
                type: 'dialogue_narrate_result',
                narration: result.narration,
                waitForUser: result.waitForUser,
              });
            } catch (error) {
              logger.error('Dialogue narration failed:', error);
              return port.postMessage({
                type: 'dialogue_narrate_error',
                error: error instanceof Error ? error.message : t('bg_cmd_dialogue_failed'),
              });
            }
          }

          case 'dialogue_form_field': {
            try {
              const providers = await llmProviderStore.getAllProviders();
              const agentModels = await agentModelStore.getAllAgentModels();
              const dialogueService = DialogueLlmService.create(providers, agentModels);
              const response = await dialogueService.formFieldRequest(
                message.fieldLabel || '',
              ...
              return port.postMessage({
                type: 'dialogue_form_field_result',
                ...response,
              });
            } catch (error) {
              logger.error('Dialogue form field request failed:', error);
              return port.postMessage({
                type: 'dialogue_form_field_error',
                error: error instanceof Error ? error.message : t('bg_cmd_dialogue_failed'),
              });
            }
          }
          ...
          case 'form_field_response': {
            if (currentExecutor && message.value !== undefined) {
              currentExecutor.resolveFormField(message.value);
            }
            break;
          }
