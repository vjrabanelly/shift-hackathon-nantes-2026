...
const OPENAI_SPEECH_TO_TEXT_MODELS = ['gpt-4o-mini-transcribe', 'gpt-4o-transcribe', 'whisper-1'];
...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
  ...
  const [modelParameters, setModelParameters] = useState<Record<AgentNameEnum, { temperature: number; topP: number }>>({
    ...
    [AgentNameEnum.Dialogue]: { temperature: 0, topP: 0 },
  });
  ...
  >({
    ...
    [AgentNameEnum.Dialogue]: undefined,
  });
  ...
  const speechToTextOptions = Object.entries(providers).flatMap(([provider, config]) => {
    const providerName = config.name || provider;

    if (config.type === ProviderTypeEnum.Gemini) {
      return (config.modelNames || []).map(model => ({
        provider,
        providerName,
        model,
      }));
    ...
    if (config.type === ProviderTypeEnum.OpenAI || config.type === ProviderTypeEnum.CustomOpenAI) {
      return OPENAI_SPEECH_TO_TEXT_MODELS.map(model => ({
        provider,
        providerName,
        model,
      }));
  ...
  const getAgentDescription = (agentName: AgentNameEnum) => {
    switch (agentName) {
      ...
      case AgentNameEnum.Dialogue:
        return t('options_models_agents_dialogue');
      default:
        return '';
    }
  };
  ...
  return (
    <section className="space-y-6">
      ...
      <div
        ...
        <div className="space-y-4">
          {[AgentNameEnum.Planner, AgentNameEnum.Navigator, AgentNameEnum.Dialogue].map(agentName => (
        ...
        <div
          ...
          <div className="flex items-center">
            ...
            <select
              ...
              {speechToTextOptions.map(({ provider, providerName, model }) => (
                <option key={`${provider}>${model}`} value={`${provider}>${model}`}>
                  {`${providerName} > ${model}`}
                </option>
              ))}
