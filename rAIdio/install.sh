#!/usr/bin/env bash
#
# rAIdio — Script d'installation complet
#
# Usage:
#   chmod +x install.sh && ./install.sh
#
# Etapes:
#   1. Verification pre-requis (uv, ollama, python3)
#   2. Dependances systeme (libportaudio2, espeak-ng)
...
step 2 "Dependances systeme (apt)"

PKGS_TO_INSTALL=()

if ! dpkg -s libportaudio2 &>/dev/null 2>&1; then
    PKGS_TO_INSTALL+=(libportaudio2)
fi

if ! command -v espeak-ng &>/dev/null; then
    PKGS_TO_INSTALL+=(espeak-ng)
fi
