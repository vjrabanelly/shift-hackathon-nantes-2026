...
exports.createAgentTurn = createAgentTurn;
exports.rejectCandidate = rejectCandidate;
exports.selectCandidatePreview = selectCandidatePreview;
exports.confirmCandidate = confirmCandidate;
const connectors_1 = require("@partyjam/connectors");
async function createAgentTurn(store, eventId, guestId, message, joystick) {
    ...
    const searchResult = await (0, connectors_1.searchMusicCandidates)({
...
async function rejectCandidate(store, eventId, guestId, candidateId) {
...
function selectCandidatePreview(store, eventId, guestId, candidateId, sourceId) {
...
function confirmCandidate(store, eventId, guestId, candidateId) {
    const latestRequest = store.getLatestRequestForGuest(eventId, guestId);
    ...
    store.appendMessage(eventId, guestId, {
        role: 'assistant',
        text: result.track.added_by === guestId
    ...
    try {
        const response = await fetch('https://api.openai.com/v1/responses', {
            ...
            body: JSON.stringify({
                ...
                instructions: "Tu es l'agent musique du front mobile d'une soirée. Réponds en français, 2 phrases maximum, style direct et chaleureux. Mentionne le meilleur candidat, explique vite pourquoi il colle, puis demande si c'est bien le morceau recherché ou s'il faut continuer.",
...
function buildFallbackAssistantMessage(input) {
    ...
    const uncertain = (topCandidate.verification_status ?? 'unverified') === 'unverified' || topCandidate.confidence < 0.55;
    if (uncertain) {
...
function selectVerifiedCandidates(message, candidates) {
    ...
    const traditionAnchor = (normalizedMessage.includes('tapis') || normalizedMessage.includes('tapie')) &&
        (normalizeText(topCandidate.title).includes('tapis') || topCandidate.genres.some((genre) => normalizeText(genre).includes('mariage')));
    const heritageContext = normalizedMessage.includes('mariage') ||
    ...
    const exactEnough = topScore >= 0.9 ||
...
function detectQueryIntents(message) {
    const normalized = normalizeText(message);
    const intents = [];
    if (normalized.includes('parole') ||
    ...
    if (normalized.includes('qui est') ||
...
function inferReferenceCandidate(store, eventId, guestId, message) {
    const normalized = normalizeText(message);
    if (!normalized.includes('son') &&
...
function buildOpenAIPrompt(input) {
    const candidates = input.candidates
        .map((candidate, index) => `${index + 1}. ${candidate.title} — ${candidate.artist}; confiance=${candidate.confidence.toFixed(2)}; genres=${candidate.genres.join(', ') || 'n/a'}; raison=${candidate.why_it_matches}`)
        .join('\n');
    return [
