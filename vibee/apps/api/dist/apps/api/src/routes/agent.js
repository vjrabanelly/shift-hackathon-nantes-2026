...
const music_agent_1 = require("../music-agent");
const store_1 = require("../store");
const track_resolver_1 = require("../services/track-resolver");
const agentStore = new store_1.PartyStore();
const agentRoutes = async (app) => {
    app.get('/:id/agent/session', async (req, reply) => {
    ...
    app.post('/:id/agent/session/reset', async (req, reply) => {
        const { id: eventId } = req.params;
    ...
    app.post('/:id/agent/turns', async (req, reply) => {
        const { id: eventId } = req.params;
        ...
        hydrateStore(snapshot);
        const result = await (0, music_agent_1.createAgentTurn)(agentStore, eventId, guest_id, message, joystick);
        const response = {
            session: result.session
        };
        return reply.send(response);
    });
    app.post('/:id/agent/candidates/:candidateId/preview/select', async (req, reply) => {
        ...
        hydrateStore(snapshot);
        const selection = (0, music_agent_1.selectCandidatePreview)(agentStore, eventId, guest_id, candidateId, source_id);
        return reply.send({
    ...
    app.post('/:id/agent/candidates/:candidateId/reject', async (req, reply) => {
        const { id: eventId, candidateId } = req.params;
        const { guest_id } = req.body;
        const snapshot = await loadEventSnapshot(eventId, guest_id);
        if (!snapshot) {
        ...
        hydrateStore(snapshot);
        const result = await (0, music_agent_1.rejectCandidate)(agentStore, eventId, guest_id, candidateId);
        return reply.send({
            session: result.session
        });
    });
    app.post('/:id/agent/candidates/:candidateId/confirm', async (req, reply) => {
        const { id: eventId, candidateId } = req.params;
        const { guest_id } = req.body;
        const snapshot = await loadEventSnapshot(eventId, guest_id);
        if (!snapshot) {
        ...
        hydrateStore(snapshot);
        const result = (0, music_agent_1.confirmCandidate)(agentStore, eventId, guest_id, candidateId);
        if (!snapshot.queueTracks.some((track) => track.id === result.track.id)) {
            const insertPayload = mapTrackForInsert(result.track);
            const { error } = await supabase_1.supabase.from('tracks').insert(insertPayload);
            if (error) {
            ...
            const hydrated = await track_resolver_1.TrackResolver.hydrateQueuedTrack({
                eventId,
...
async function loadEventSnapshot(eventId, guestId) {
    const [{ data: event }, { data: guest }] = await Promise.all([
        supabase_1.supabase.from('events').select('*').eq('id', eventId).single(),
        supabase_1.supabase.from('guests').select('*').eq('id', guestId).eq('event_id', eventId).maybeSingle()
    ]);
    if (!event) {
        return null;
    }
    const [{ data: playingTrack }, { data: queuedTracks }, { data: guests }, { data: joystickPositions }] = await Promise.all([
        supabase_1.supabase.from('tracks').select('*').eq('event_id', eventId).eq('status', 'playing').maybeSingle(),
        supabase_1.supabase.from('tracks').select('*').eq('event_id', eventId).eq('status', 'queued').order('position', { ascending: true }),
        supabase_1.supabase.from('guests').select('id, display_name, emoji').eq('event_id', eventId),
        supabase_1.supabase.from('joystick_positions').select('valence, energy').eq('event_id', eventId)
    ]);
    ...
    const queue = queueTracks.map((track) => ({
        track,
        position: track.position,
        added_by_display: track.added_by === 'ai'
            ? '🤖 IA'
...
function computeCollective(positions) {
    ...
    const totals = positions.reduce((accumulator, current) => ({
...
function inferPrimarySource(track) {
...
function inferLinkedSources(track) {
...
function mapTrackForInsert(track) {
    return {
        id: track.id,
