...
function buildStreamUrl(request, eventId) {
...
async function eventsRoutes(app) {
    app.post('/', async (request, reply) => {
        ...
        const adminToken = 'tok_' + crypto_1.default.randomBytes(16).toString('hex');
        ...
        if (body.seed_playlist && body.seed_playlist.length > 0) {
            ...
            const { data: insertedTracks, error: tracksError } = await supabase_1.supabase
                .from('tracks')
                .insert(tracks)
                .select();
            if (tracksError) {
                ...
                for (const track of insertedTracks ?? []) {
                    track_resolver_1.TrackResolver.hydrateQueuedTrack({
                        eventId: event.id,
                        trackId: track.id,
                        title: track.title,
                        artist: track.artist,
    ...
    app.get('/:id', async (req, reply) => {
        ...
        const { data: queuedTracks } = await supabase_1.supabase
            .from('tracks')
            .select('*')
            .eq('event_id', id)
            .eq('status', 'queued')
        ...
        const { data: guests } = guestIds.length > 0
            ? await supabase_1.supabase
                ...
                .in('id', guestIds)
            : { data: [] };
        const guestMap = new Map((guests ?? []).map((g) => [g.id, `${g.emoji ?? '🎵'} ${g.display_name}`]));
        const queue = (queuedTracks ?? []).map((track) => ({
            track,
        ...
        const { count: guest_count } = await supabase_1.supabase
            .from('guests')
        ...
        const { data: joystickPositions } = await supabase_1.supabase
            ...
            .eq('event_id', id);
        const collective_joystick = (0, compute_collective_1.computeCollective)(joystickPositions ?? []);
        const { data: playedTracks } = await supabase_1.supabase
            .from('tracks')
            .select('*')
            .eq('event_id', id)
            .eq('status', 'played')
            ...
            .limit(20);
        const history = (playedTracks ?? []).map((track) => ({
            track,
            position: track.position,
            added_by_display: track.added_by === 'ai'
                ? '🤖 IA'
    ...
    app.get('/:id/now-playing/audio', async (req, reply) => {
        ...
        if (!range) {
            ...
            return reply.send(node_fs_1.default.createReadStream(filePath));
        }
        ...
        return reply.send(node_fs_1.default.createReadStream(filePath, { start, end }));
    });
    ...
    app.post('/:id/history/replay', async (req, reply) => {
        const { id: eventId } = req.params;
        const adminToken = req.headers['x-admin-token'];
        const { data: event } = await supabase_1.supabase
            .from('events')
        ...
        const { data: lastQueued } = await supabase_1.supabase
            .from('tracks')
            .select('position')
            .eq('event_id', eventId)
            .eq('status', 'queued')
    ...
    app.delete('/:id/tracks/:trackId', async (req, reply) => {
        ...
        const { data: track } = await supabase_1.supabase
            .from('tracks')
            .select('id, status')
            .eq('id', trackId)
            .eq('event_id', eventId)
...
async function ensureNowPlayingTrack(eventId) {
    const { data: currentPlaying } = await supabase_1.supabase
        .from('tracks')
        .select('*')
        .eq('event_id', eventId)
        .eq('status', 'playing')
